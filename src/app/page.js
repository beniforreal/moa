...
export default function App(){
  ...
  return <div className="shell">
        ...
        {view==='settings'&&<>
          ...
          <div className="client-channel-stack">
            {filteredClients.map(c=><section className="panel client-channel-panel" key={c.id}>
              <div className="client-channel-head"><div><span className="eyebrow">CLIENT CHANNELS</span><h2>{c.name}</h2><p className="muted">{c.category||'업종 미등록'}</p></div><button className="secondary compact" onClick={()=>{setSelected('');setView('inbox')}}>댓글함 보기</button></div>
    ...
    {modal&&<div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}><section className={'modal '+(modal.type==='post'?'wide':'')}>
      ...
      {modal.type==='post'&&<form onSubmit={savePost}><span className="eyebrow">CONTENT</span><h2>{modal.item?'콘텐츠 수정':'게시물 작성'}</h2><div className="form-grid"><label>고객사<select name="client_id" required defaultValue={modal.item?.client_id||(client==='all'?data.clients[0]?.id:client)}>{data.clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>채널<select name="platform" defaultValue={modal.item?.platform||'instagram'}><option value="instagram">Instagram</option><option value="naver">네이버 블로그</option></select></label></div><label>제목<input name="title" required defaultValue={modal.item?.title||''}/></label><label>본문<textarea name="body" rows={8} required defaultValue={modal.item?.body||''}/></label><label>Instagram 이미지 공개 URL<input name="image_url" type="url" placeholder="https://..." defaultValue={modal.item?.image_url||''}/></label><label>예정일<input name="scheduled_at" type="datetime-local" defaultValue={localInput(modal.item?.scheduled_at)}/></label><button className="primary full" disabled={busy}>저장</button></form>}
